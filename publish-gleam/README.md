...
| `expected-version` | no | `""` | Release version (leading `v` and a `-dryrun-<sha>` suffix stripped). When set, `gleam.toml` must declare it or the action fails before publishing. |
