...
This repository holds reusable GitHub Actions and workflows. The security-relevant surface is **supply chain**: an action that leaks a token, over-broad `permissions:`, an unpinned third-party action, or a step that interpolates untrusted input (a PR title, a branch name) directly into a shell command. Report any of these.
