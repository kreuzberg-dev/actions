...
| `expected-version` | no | `""` | Release version (leading `v` and a `-dryrun-<sha>` suffix stripped). When set, `pubspec.yaml` must declare it or the action fails before publishing. |
